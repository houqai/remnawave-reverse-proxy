...
if { [ -t 0 ] || [ -t 1 ]; } && [ -z "${NO_COLOR:-}" ] && [ "${TERM:-}" != "dumb" ]; then
    ...
    GREEN='\033[38;2;127;176;105m'   # success green (status glyphs)
    YELLOW='\033[38;2;221;181;126m'  # warm sand (warnings)
    BLUE='\033[38;2;217;119;87m'     # remapped to coral (primary accent)
    CYAN='\033[38;2;110;110;110m'    # remapped to dim (rules)
    BOLD='\033[1;38;2;217;119;87m'   # bold coral (section titles)
else
    NC=''; CORAL=''; RED=''; GREEN=''; YELLOW=''; BLUE=''; CYAN=''; BOLD=''
fi
